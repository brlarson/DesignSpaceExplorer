 ...
 * @author phf
 */
public class MyInstantiateModel extends InstantiateModel {
...
//	}
	 ...
	 * @param errMgr
	 */
	public MyInstantiateModel(final IProgressMonitor pm) {
	  super(pm);
...
//		monitor = pm;
	...
	public MyInstantiateModel(final IProgressMonitor pm, final AnalysisErrorReporterManager errMgr) {
	  super(pm,errMgr);
//		classifierCache = new HashMap<InstanceObject, InstantiatedClassifier>();
//		mode2som = new HashMap<ModeInstance, List<SystemOperationMode>>();
//		errManager = errMgr;
...
//		URI eobj = InstantiateModel.getInstanceModelURI(ci)
//		if (eobj instanceof ComponentImplementation) {
//			ici = (ComponentImplementation) eobj;
//		}
//		URI instanceURI = InstantiateModel.getResourceURI(ici);
//		Resource aadlResource = OsateResourceUtil.getEmptyAaxl2Resource(instanceURI);// ,si);
		// now instantiate the rest of the model
		final MyInstantiateModel instantiateModel = new MyInstantiateModel(monitor, new AnalysisErrorReporterManager(
				new MarkerAnalysisErrorReporter.Factory(AadlConstants.INSTANTIATION_OBJECT_MARKER)));
		
		SystemInstance root = instantiateModel.createSystemInstance(ci, ici);
		if (root == null) {
			setErrorMessage("Null root returned from createSystemInstance");
		}
		return root;
	}
...
//		res.save(null);
//		res.unload();
//		//FIXME?? OsateResourceUtil.refreshResourceSet();
//		ci = (ComponentImplementation) OsateResourceUtil.getResourceSet().getEObject(uri, true);
//		final MyInstantiateModel instantiateModel = new MyInstantiateModel(new NullProgressMonitor(),
//				new AnalysisErrorReporterManager(
//						new MarkerAnalysisErrorReporter.Factory(AadlConstants.INSTANTIATION_OBJECT_MARKER)));
//		SystemInstance root = instantiateModel.createSystemInstance(ci, res);
//
//		return root;
		return InstantiateModel.rebuildInstanceModelFile(ires);
	}
...
//				throw e;
//			} catch (Exception e) {
//				MyInstantiateModel.setErrorMessage(e.getMessage());
//				e.printStackTrace();
//				return null;
//			}
//		} catch (IOException e) {
...
//		MyCacheContainedPropertyAssociationsSwitch ccpas = new MyCacheContainedPropertyAssociationsSwitch(classifierCache,
//				scProps, monitor, errManager);
//		ccpas.processPostOrderAll(root);
//		if (monitor.isCanceled()) {
//			throw new InterruptedException();
//		}
//		final MyCachePropertyAssociationsSwitch cpas = new MyCachePropertyAssociationsSwitch(monitor, errManager,
...
//hacked by BRL 9/17/2022				
        Context srcpg = (Context)srcfp;
				if (srcpg == null) {
