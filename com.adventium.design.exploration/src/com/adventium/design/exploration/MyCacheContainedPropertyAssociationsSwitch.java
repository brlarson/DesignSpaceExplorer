 ...
 * @author lwrage
 */
public class MyCacheContainedPropertyAssociationsSwitch extends AadlProcessingSwitchWithProgress {
	...
	protected MyCacheContainedPropertyAssociationsSwitch(
				...
				if (cpes != null && !cpes.isEmpty()) {
					...
					for (InstanceObject io : ios) {
						...
						if (last instanceof Connection) {
							...
							io.removePropertyAssociations(prop);
							io.getOwnedPropertyAssociations().add(newPA);
							System.err.println("Hay!  Adding #1.B "+io.getFullName()+"  "+ prop);
						}
					}
				}
			}
